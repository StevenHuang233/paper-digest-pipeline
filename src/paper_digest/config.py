...
DEFAULTS: dict[str, Any] = {
    ...
    "discovery": {
        "source": "arxiv", "date": "today", "max_candidates": 200,
        "page_size": 100, "request_delay_seconds": 3.1, "json_path": "",
    ...
    "selection": {"ranker": "rules", "min_score": 0.0, "max_papers": 5, "llm_batch_size": 20},
    "backend": {
    ...
    "budget": {
        "max_total_tokens": 80000, "max_estimated_usd": 5.0,
...
def validate_config(config: dict[str, Any], *, require_backend: bool = True) -> None:
    ...
    if config["selection"]["max_papers"] < 1:
        raise ValueError("selection.max_papers must be >= 1")
